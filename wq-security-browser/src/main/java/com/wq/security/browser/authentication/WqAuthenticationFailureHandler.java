...
import com.wq.security.browser.support.SimpleResponse;
...
public class WqAuthenticationFailureHandler extends SimpleUrlAuthenticationFailureHandler/*implements AuthenticationFailureHandler*/{
    ...
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException exception) throws IOException, ServletException {
        ...
        if (LoginType.JSON.equals(securityProperties.getBrowser().getLoginType())) {
            ...
            response.getWriter().write(objectMapper.writeValueAsString(new SimpleResponse(exception.getMessage())));//exception返回太全不需要这样
        }else{
